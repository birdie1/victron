...
class Phoenix:
    ...
    def format_data(self, data):
        ...
        new_dict['product'] = f"{data['SER#']}: {self.PID[data['SER#']]}"
        new_dict['mode'] = f"{data['MODE']}: {self.MODE[data['MODE']]}"
        new_dict['state'] = f"{data['CS']}: {self.CS[data['CS']]}"
        ...
        if int(data['AC_OUT_I']) <= 0:
            ...
            cur = int(data['AC_OUT_I']) / self.DEVIDER['AC_OUT_I']
        new_dict['ac_current'] = f"{cur:2f}"

        new_dict['alarm_reason'] = self.extract_warn_ar(data['AR'])
        new_dict['warning'] = self.extract_warn_ar(data['WARN'])
